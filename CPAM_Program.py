...
def visits_by_fiscal_year():
  ...
  plot.set_xlabel('Fiscal Years')
  plot.set_ylabel('Visits')
  plot.set_title('Visits by Fiscal Year')
...
#Adds button to save the visualization
  save = ttk.Button(root, text="Save Chart", command=lambda: save_chart(fig, 'VisitsbyFiscalYear'))
...
#Adds button for visits by year visualization
  visits_by_fiscal_year_button = ttk.Button(root, text="Visits by Fiscal Year", width = 32, command=visits_by_fiscal_year)
  visits_by_fiscal_year_button.grid(row = 0, column = 1, padx = 20, pady = 20)
